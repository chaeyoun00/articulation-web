						...
						<template v-slot:[`item.setting`]="{ item }">
							...
							>
								...
								<v-card
									...
									"
								>
									<v-card-title> 환자 삭제 </v-card-title>
									<v-card-text
...
<script>
	...
	export default {
		...
		methods: {
			...
			Delete(item) {
				var url = this.$API_SERVER + '/api/examUsers?id=' + item.u_id;
...
<style>
	...
	.numselect.theme--light.v-text-field--solo
		> .v-input__control
		> .v-input__slot {
	...
	.main-table.theme--light.v-data-table
	...
	.search-text.theme--light.v-text-field--solo
		> .v-input__control
		> .v-input__slot {
		width: 420px;
		height: 46px;
