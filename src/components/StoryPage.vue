				...
				<tbody v-for="i in questions.length" v-bind:key="i">
					<tr v-if="i == 1">
						<td
							...
							style="border-radius: 21px 21px 0px 0px"
						>
							{{ i }}번째 그림
							<audio
					...
					<tr v-else-if="i == questions.length">
						<td
							class="story-table-title"
							style="border-radius: 0px 0px 21px 21px"
						>
							{{ i }}번째 그림
							<audio
								:id="questions[i - 1].q_id"
		...
		methods: {
			async initialize() {
				...
				const configNoImage = {
					...
					url:
						this.$API_SERVER +
						'/api/questions/noimage?type=Story_Telling',
				};
				...
				const configAnswer = {
					...
					url:
						this.$API_SERVER +
						'/api/answerPapers?type=Story_Telling&examId=' +
						this.resId,
				};
				...
				const configSummary = {
					...
					url:
						this.$API_SERVER +
						'/api/languageSummary?type=Story_Telling&userId=' +
						this.user[0].u_id +
						'&resId=' +
						this.resId,
				};
				...
				await axios(configSummary)
					.then((response) => {
						...
						this.image = this.storyAnswer[0].lg_answer
							.slice(1, -1)
							.split(',');
						this.validated = 0;
					})
