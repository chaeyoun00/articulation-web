	...
	<v-container>
		...
		<v-layout justify-center column class="cread-form2">
			...
			<table class="cread-table1">
				<tr>
					<td class="cread-table-header"
						><div class="rect">TESTS</div></td
					>
					<td class="cread-table-header1">Total score (raw)</td>
					<td
						class="cread-table-header1"
			...
			<table class="cread-table2">
				...
				<tr style="border-bottom: 1px solid #c9c9c9">
					<td class="cread-table-content2"
						>J1. 언어유창성 검사: "동물 범주"</td
					>
					<td class="cread-table-content1">
						<div class="cread-score2">
							<input
			...
			<p class="test-title" style="margin-top: 87px"
				>CREAD-K (Clinical)</p
			>
			<table class="cread-table1">
				<tr>
		...
		methods: {
			...
			async initialize() {
				...
				const configRecognition = {
					...
					url:
						this.$API_SERVER +
						'/api/recognitionSummary?type=CREAD-K&resId=' +
						this.resId,
				};
				...
				await axios(configRecognition)
					.then((response) => {
						if (response.data.data.length > 0) {
							this.scores = response.data.data[0].rs_answer
								.slice(1, -1)
								.split(',');
...
/* .v-data-table__wrapper > table > tbody > tr:hover { background: inherit
!important; } */
