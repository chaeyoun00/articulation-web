					...
					<v-flex>
						...
						<div class="file">
							<input
								...
								accept="image/*"
							/>
							<label class="file_label" for="pic_file"
								>파일 첨부</label
							>
							<input
								type="file"
						...
						<div class="file">
							<input
								...
								accept="image/*"
							/>
							<label class="file_label" for="pic_file1"
								>파일 첨부</label
							>
							<input
								type="file"
	...
	export default {
		...
		methods: {
				...
				axios(config)
					.then((response) => {
						...
						if (
							JSON.parse(
								iconv.decode(
									response.data.data[0].q_data.data,
									'UTF-8'
								)
							)['type_of_question'] === 'ex'
						) {
						...
						this.text = response.data.data[0].q_body.replace(
							/,/g,
							' '
						);
						this.id = JSON.parse(
			...
			AddQuestion() {
				if (this.type == '문항') {
					this.type = 'qt';
				} else if (this.type == '연습 문항') {
					this.type = 'ex';
				} else if (this.type == '예시 문항') {
					this.type = 'word';
				}
...
<style>
	...
	.question-select.theme--light.v-text-field--solo
		> .v-input__control
		> .v-input__slot {
		width: 453px;
		height: 49px;
