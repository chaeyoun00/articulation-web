				...
				await axios(config)
					.then((response) => {
						...
						for (let i = 0; i < this.lists.length; i++) {
							this.lists[i].q_body = this.lists[i].q_body.replace(
								/,/g,
								' '
							);
							this.lists[i].q_data = String.fromCharCode(
								...this.lists[i].q_data.data
							);
							...
							if (
								JSON.parse(this.lists[i].q_data)[
									'type_of_question'
								] === 'word'
							) {
								if (
									JSON.parse(this.lists[i].q_data)['no'] ==
									'00'
								) {
									this.lists[i].q_id = 0;
								} else {
									this.lists[i].q_id = JSON.parse(
										this.lists[i].q_data
									)['no'].replace(/(^0+)/, '');
								}
