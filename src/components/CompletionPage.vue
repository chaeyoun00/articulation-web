		...
		methods: {
			...
			async initialize() {
				...
				const configNoImage = {
					...
					url:
						this.$API_SERVER +
						'/api/questions/noimage?type=SPT-completion',
				};
				await axios(configNoImage)
					.then((response) => {
						...
						for (let i = 0; i < this.questions.length; i++) {
							...
							this.answers[i] = JSON.parse(contents)[
								'answer'
							].replace(/,/g, ' ');
							if (this.qtype[i] === 'ex') {
								this.num[i] =
				...
				const configAnswer = {
					...
					url:
						this.$API_SERVER +
						'/api/answerPapers?type=SPT-completion&examId=' +
						this.resId,
				};
				...
				const configSummary = {
					...
					url:
						this.$API_SERVER +
						'/api/languageSummary?type=SPT-completion&userId=' +
						this.user[0].u_id +
						'&resId=' +
						this.resId,
				};
