			...
			<table class="cmpt-table">
				...
				<tbody v-for="i in questions.length" v-bind:key="i">
					<tr class="cmpt-table-content">
						<td>{{ num[i - 1] }}</td>
						<td style="background-color: #f5f5f5; width: 279px">{{
							answers[i - 1]
						}}</td>
						...
						>
							{{ check[i - 1] }} </td
						><td v-else class="cmpt-table-blank"></td>
						<td
				...
				<tbody>
					<tr class="cmpt-table-content1">
						<td class="cmpt-table-end" colspan="3"
							>문장유형 별 합계</td
						>
					...
					<tr>
						...
						>
						<td
							class="cmpt-table-end1"
							colspan="6"
							style="border-radius: 0px 0px 21px 0px"
							>{{ totalscore }} / {{ totalquestion }}</td
						>
					</tr>
				</tbody>
			</table>
		...
		methods: {
			...
			async initialize() {
				...
				await axios(configAnswer)
					.then((response) => {
						for (let i = 0; i < response.data.data.length; i++) {
							this.cmptAnswer[i] = response.data.data[
								i
							].a_answer.replace('  ', ' ');
						}
				...
				await axios(configNoImage)
					.then((response) => {
						...
						for (let i = 0; i < this.questions.length; i++) {
							sentence = iconv.decode(
								this.questions[i].q_data.data,
								'UTF-8'
							);
							if (
								JSON.parse(sentence)['type_of_question'] ===
								'word'
							) {
							...
							this.answers[i] = JSON.parse(sentence)[
								'answer'
							].replace(/,/g, ' ');
							...
							if (
								...
								this.num[i] =
									'P' +
									JSON.parse(sentence)['no'].replace(
										/(^0+)/,
										''
									);
							} else {
								this.num[i] = JSON.parse(sentence)[
									'no'
								].replace(/(^0+)/, '');
								this.scores[
