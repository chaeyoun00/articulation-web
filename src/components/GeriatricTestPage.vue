	...
	<v-container>
		...
		<v-layout justify-center column class="geriatric-form2">
			...
			<table class="geriatric-table1">
				<tr>
					<td class="geriatric-table-header1"
						><div class="rect"
					/></td>
		...
		<v-layout justify-center class="geriatric-form3">
			<v-btn depressed class="submit-btn" @click="save(), toTest()"
				>저장</v-btn
			>
		</v-layout>
	</v-container>
		...
		methods: {
			...
			async initialize() {
				...
				const configRecognition = {
					...
					url:
						this.$API_SERVER +
						'/api/recognitionSummary?type=Geriatric_Depression&resId=' +
						this.resId,
				};
				...
				await axios(configRecognition)
					.then((response) => {
						...
						this.yes = response.data.data[0].rs_answer
							.slice(1, -1)
							.split(',')[0];
						this.no = response.data.data[0].rs_answer
							.slice(1, -1)
							.split(',')[1];
